...
(async () => {
    ...
    const HC = (window.HotCat = {
        ...
        isSupportedContentModel: () =>
            HC.wikitextContentModels.includes(conf.wgPageContentModel) ||
    ...
    const change_category_wikitext = (_wikitext, toRemove, toAdd, _key, is_hidden) => {
        ...
        if (toAdd && toAdd.length) {
            ...
            if (cat_point >= 0) {
                ...
                wikitext =
                    wikitext.substring(0, cat_point) +
                    (cat_point > 0 ? '\n' : '') +
                    newcatstring +
                    (!onCat ? '\n' : '');
    ...
    class CategoryEditor {
        ...
        makeForm() {
            ...
            const button_label = (id, defaultText) => {
                ...
                if (
                    ...
                    window.UFUI.getLabel instanceof Function
                ) {
                    try {
        ...
        accept(evt) {
            ...
            if (this.acceptCheck()) {
                ...
                void resolveMulti(toResolve, resolved => {
                    if (resolved[0].dab) {
                        showDab(resolved[0]);
                    } else {
        ...
        commit() {
            if (
                (this.currentCategory === this.originalCategory &&
                    (this.currentKey === this.originalKey || (this.currentKey === null && !this.originalKey.length))) ||
            ...
            if (changes === 1) {
                ...
                for (i = 0; i < changed.length; i++) {
                    if (changed[i].from !== changed[i].to) {
                        summary.push(
                            `±${substitute(HC.messages.short_catchange, [null, changed[i].from])}${arrow}${substitute(HC.messages.short_catchange, [null, changed[i].to])}`,
                        );
                    } else {
                ...
                if (changed.length === 1) {
                    if (changed[0].from !== changed[0].to) {
                        shortSummary.push(
                            `±${substitute(HC.messages.short_catchange, [null, changed[0].from])}${arrow}${substitute(HC.messages.short_catchange, [null, changed[0].to])}`,
                        );
                    } else {
    ...
    const makeActive = which => {
        ...
        if (which.dab) {
            ...
            if (
                (!expectedInput.length && actualValue.length) ||
                (expectedInput.length && actualValue.indexOf(expectedInput))
            ) {
    ...
    const list_categorys = () => {
        ...
        const cat_regex = new RegExp(
            `\\[\\[${wikiTextBlankOrBidi}(${HC.category_regexp})${wikiTextBlankOrBidi}:${wikiTextBlankOrBidi}([^\\[\\]]+?)${wikiTextBlankOrBidi}(\\|.*?)?\\]\\]`,
            'g',
        );
    ...
    const getCatLine = () =>
        skin === 'moeskin' ? document.getElementById('catlinks') : document.getElementById('mw-normal-catlinks');
    const setup = additionalWork => {
        ...
        const hiddenCats =
            skin === 'moeskin'
                ? document.querySelector('.mw-catlinks-hidden')?.parentElement
        ...
        const createEditors = (_line, is_hidden, has_hidden) => {
            ...
            for (i = 0; i < inpageCats.length; i++) {
                ...
                if (Reflect.has(idxCopys, normalized) && line) {
                    new CategoryEditor(
                        line,
                        copyCats[idxCopys[normalized]],
                        inpageCats[i].match[2],
                        inpageCats[i].match[3],
                        is_hidden,
                    );
            ...
            if (chkCats.length && ((!is_hidden && !has_hidden) || is_hidden)) {
                ...
                $.getJSON(
                    `${conf.wgServer}${conf.wgScriptPath}/api.php?action=query&format=json&titles=${encodeURIComponent(arg_titles)}&redirects=1&converttitles=1`,
                    json => {
                        ...
                        for (i = 0; i < converted.length; i++) {
                            ...
                            if (Reflect.has(idxCopys, sTitle) && line) {
                                new CategoryEditor(
                                    line,
                                    copyCats[idxCopys[sTitle]],
                                    [inpageCats[sOrginal].match[2], sTitle],
                                    inpageCats[sOrginal].match[3],
                                    is_hidden,
                                );
                            } else {
                                console.log(sTitle);
        ...
        new CategoryEditor(
            newDOM ? catLine.getElementsByTagName('ul')[0] : catLine,
            null,
            null,
            lastSpan !== null,
            false,
        ...
        if (
            ...
            if (!HC.isSupportedContentModel()) {
                const issueURL = new URL(
                    'https://github.com/MoegirlPediaInterfaceAdmins/MoegirlPediaInterfaceCodes/issues/new?template=BLANK_ISSUE',
                );
                issueURL.searchParams.set('title', '[HOTCAT] 未知的 contentModel');
                issueURL.searchParams.set(
                    'body',
                    `页面 <${new URL(`/${conf.wgPageName}`, location.href)}> 使用了未知的 contentModel \`${conf.wgPageContentModel}\`，导致 HotCat 无法正常工作。请调查这个问题。`,
                );
                console.warn(
                    `[HOTCAT] 本页面 <${conf.wgPageName}> 使用了未知的 contentModel "${conf.wgPageContentModel}"。如果你看到此信息，请报告此问题至 ${issueURL.toString()}`,
                );
            }
            getPage();
        }
